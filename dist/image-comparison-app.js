...
<button @click="${() => this.reset()}" class="no-border">
  Clear selections
            ...
            instructions = $ `<p>Select two images to compare them!</p>`;
        }
        return $ `
<style>
:host {
...
class RenderingScenario extends s {
    ...
    metricTemplate(analysisResults, goldenName) {
        ...
        return $ ` <span>${rmsInDecibel} dB </span>
      <div class="tooltip">
...
class ImageComparisonApp extends s {
    ...
    render() {
        const { config } = this;
        if (config == null) {
            return this.src ? $ `Loading...` : $ `No config specified`;
        }
